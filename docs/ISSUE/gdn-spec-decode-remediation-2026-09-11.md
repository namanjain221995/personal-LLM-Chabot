...
| Commit (subject; hashes change on rebase/merge, subjects do not) | What |
|---|---|
| `docs(issue): keep the GDN spec-decode root-cause report in the repository` | The analysis was an untracked file and was lost from the working tree during a branch switch by another session on 2026-09-11; restored verbatim and tracked. |
| `launcher: speculative decoding is opt-in, prefix caching is a real switch` | `CLUSTER_SPECULATIVE_CONFIG` absent/empty ⇒ no flag at all (was: absent ⇒ MTP on); the MTP JSON is an explicit opt-in. One validator (`cluster.speculative_config_argument`) renders both the dual-mode `CLUSTER_ENGINE_ARGS` and a new `MAIN_MODEL_SPECULATIVE_ARGUMENT` for the single-node overlays, so `CLUSTER_MODE=auto` degrading to single can no longer bring MTP back by itself. `MAIN_MODEL_ENABLE_PREFIX_CACHING` becomes a real switch (`--enable-prefix-caching` / `--no-enable-prefix-caching`) on both ranks and in `compose.dgx-spark.yaml` / `compose.nvidia.yaml`, re-emitted normalised into `generated.env` so the orchestrator's `/health` belief is the launch. The overlays default the two new keys to empty so a `generated.env` from an older launcher still renders. |
| `launcher: say exactly which faults, which default, which timeout` | Narrative corrections + the stale-`generated.env` overlay test. |
| `orchestrator: model calls wait out an engine restart instead of dying on it` | `app/resilience.py` and the video deferral (§4). |
| `scripts: prove the engine on both nodes, and soak the pair with a mixed workload` | `scripts/cluster-verify-engine.sh`, `scripts/cluster-soak.py`. |
| `compose: explicit KV budgets for the embed and reranker engines` | §8. |
| `whisper: a replica whose CUDA context died says so and restarts` | §8. |
| `orchestrator: nothing produced is not a partial answer; one "restarting" line per turn` | Found by failure test A (§10). |
| `compose: the worker re-pairs after 2 minutes of an unreachable head, not 5` | §8. |
| `scripts: the soak harness backs off on a dead port instead of hammering it` | §7. |
| `orchestrator: the deferral budget counts deferrals, and the retry delay is real` | Review follow-ups: `counts["deferrals"]` instead of the lifetime `attempt`; a timer re-drains after `VIDEO_RETRY_DELAY_S` instead of waiting for the 30-min maintenance tick; a deferred stage renders as a waiting step with "will be retried", not as done. |
| `docs: …` (two commits) | This record, the runbook, CHANGELOG, README corrections, the interview-analysis patches. |
...
- Video pipeline: a required stage that cannot reach its engine (`ModelUnavailable`, `ASRUnavailable`, `ASRBusy`) **defers** the job — status back to `queued`, `error="waiting for the model (attempt N)…"`, finished stages kept, the deferred stage not stamped done — bounded by `VIDEO_MAX_ATTEMPTS`=5 deferrals (counted in the row's `counts`, so re-runs do not consume the budget), and re-drained after `VIDEO_RETRY_DELAY_S`=300 s by a timer armed at the deferral (the maintenance tick alone is 30 min apart). An optional stage fails soft. An OCR-engine outage is no longer cached as "no readable text" for the life of those bytes. Idempotency: stage outputs are content-hash-keyed files plus the row's `stages` map; the V29 lease prevents two runners on one video.
...
Engine only, ~6 minutes, no code change: in `.env` set `CLUSTER_SPECULATIVE_CONFIG='{"method":"mtp","num_speculative_tokens":1}'` and/or `MAIN_MODEL_ENABLE_PREFIX_CACHING=true`, then `./techsara up` from a shell on the head (or `scripts/deploy.sh --full`), then `scripts/cluster-verify-engine.sh --probe` (it will report the flags it finds). The launcher's previous behaviour (MTP on when the key is absent) is a `git revert` of the two `launcher:` commits above — but the `.env` line is the switch that matters, and it is explicit either way now. The orchestrator change is independent and rolls with a routine deploy; `LLM_INTERACTIVE_RECOVERY_S=0 LLM_RECOVERY_WINDOW_S=0` in `.env` turns the waiting off without a code change.
