...
## 7c. The material (since 2026-09-17)

A file turn is made FROM what `artifacts/material_in.gather` collects: the most recent SUBSTANTIAL answer with its markdown, this turn's attachments, that answer's tables, figures typed in the request — and **the conversation's own datasets**. When the turn has no attachment, every ready CSV/XLSX upload of the conversation (an `uploads` row whose `notes` are not `document` or `video`) is read as `upload1`, `upload2`, … The most recent one, or the one the words name, is read first. This happens for every create except an export or a `previous_answer` target, and for every edit, whatever `intent.target` says: that target is only ever `upload` when a file is attached to THIS turn, so before this rule a CSV uploaded one turn earlier never reached the composer and every report over it was invented. The budget is unchanged: `MAX_UPLOAD_ROWS` (200,000) across at most five files, inside `GATHER_DEADLINE_S` (15 s), every reader in a worker thread. An upload that is not `ready` is skipped with a note; when the bytes are gone but the stored profile kept `full_rows`, those rows stand in. The gather step names what it read — `customers-100.csv, 100 rows` — never "from the conversation".

## 7d. Charts a person asks for (since 2026-09-17)

**No data, no job.** When every selected format is `png` or `svg` and the material holds no table, the turn is answered in one sentence asking for the data as a table (naming any file that could not be read) and NO job is accepted. A png/svg job whose charts all became "could not be drawn" callouts fails with that callout's own sentence, not "The artifact has no charts to draw as images".

**Charts chosen by code.** `chart_choice.suggest_charts(table, instruction=…, limit=3)` returns bindings — never numbers — in this order: date columns first (rows counted per month, quarter or year, chosen from the span), then text columns whose distinct count is at most half the rows and whose biggest group holds at least three, then numeric measures. A column that NAMES its rows is never suggested: `distinct >= 0.8 × rows`, or a name like id, email, phone, website or url. It is used when a png/svg request's resolved spec has no chart and the request names no chart type or column, and when an `add_chart` edit names no column. When the person DID name a type or a column that failed to bind, the sentence says which column is missing and lists the table's columns; a suggestion never replaces what they asked for.

**Tail and guard.** A `bar`/`horizontal_bar` binding over more than 15 categories with no `top_n` gets `top_n = 10` with an Other bucket, and the version says so (`Binding.top_n` is the category CAP, so a cap of 10 draws the 9 largest beside Other); a `top N` the person typed wins. A COUNTING chart (`agg='count'`, no measure) is not drawn at all when its x column is near-unique or its tallest bar holds two rows — the sentence names the column and why. A chart with a real measure is never refused this way: one row per product is how a product table looks.

**`add_chart`.** `artifacts/edits.py` has an `add_chart` op — `{after?, chart_type?, x?, y?, agg?, title?, table_id?, count, instruction}` — documented in `_PLAN_SYSTEM`, with the turn's tables reaching the planner prompt as SHAPE only (ids, row counts, column names, the values of low-cardinality columns; never a data row). The pre-planner maps a pure chart request ("add a chart", "also i want Plots on this docs", "include graphs", "add a bar chart after Geography") to it with zero model calls: one chart for a singular request, up to three for a plural one. Apply binds only columns that exist and otherwise uses the suggestions; it replaces a "the table … is not available" callout in place, else inserts after the named section, else opens a `Charts` section before a closing conclusion/recommendations section. The compose stage computes the numbers. With no table the op is not applied, the reason is given and no version is saved. A document written under ONE top-level heading is addressed at its H2s, and the H1 keeps its own name.

**Honest sentences.** An edit whose words are about charts but whose PUBLISHED version gained none never says a chart was added; it says `Not applied: the chart (…)`.
