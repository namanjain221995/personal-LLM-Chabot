...
| **restart drill** (`--restart-during-c`) | the orchestrator container restarted 7 s into the compose; the stream broke; the job was requeued and published with all four files, 23/23 checks. Two drills: published 26 s after the restart, and (final tree) requeued 95 s after startup — the bound is the lease TTL (90 s) plus the 30 s check, counted from the last heartbeat before the restart. The first drill sat for 15 minutes: the startup pass ran before the dead lease had expired and the next pass was the 30-minute sweep — fixed (30 s lease check) |

**Final run on the committed tree (c54c590, after the security round): A–E PASS, 74/74 checks, 124 s wall clock** — A 14.7 s, B 10.2 s + 14.3 s (3-page PDF, every section present), C 20.3 s, D 4.5 s + four text answers 1.8–3.8 s, E 11.3 s / 17.8 s / 1.2 s; then the restart drill PASS 23/23.

### Security review (adversarial, 2026-09-12)

Eight surfaces (the intent hook, paste parsing, generator recipes, rewrite batches, the CSV writer, the file/zip routes, pipeline leases, spec validation) were attacked by independent agents and every finding was refuted by two more; 101 attacks held, 37 findings, **19 confirmed and fixed** (commit c54c590), the rest refuted or duplicates. The two high findings: the classifier hook ran regexes over the full text of a turn (now the first `_DECIDE_CHARS` only — a 100 KB adversarial message decides in 3 ms), and pasted tables were parsed on the event loop (now `asyncio.to_thread` under `MAX_PASTE_BYTES`, newest turn first; the test measures the loop's longest stall under 0.25 s). Also: `{n:0999999999d}` allocated a gigabyte in the spec's own `format(n=1)` probe (patterns are now read with `string.Formatter`, width ≤ 40, before anything formats them); `TABULAR_MAX_CHARS` lowered from 2 M to 400 k (1.98 M chars of 50-char cells took 510 s in-process against the 180 s render window); negated clauses and formats ("not a PDF", "I don't want a PDF") no longer create or pick the format, while first-person inability ("I can't make a spreadsheet myself, can you build one?") still creates; the row count is read from the prose the parser puts before a table, never from a cell; file ids are validated by shape before any lookup; the zip is bounded (413) and refusals are 409; over-budget material is refused on retry as well as accept.

Documented limits (not fixed): a rewrite that inverts meaning with no negation, figure or quoted span changed is not caught; homonyms ("directions to cvs") can read as a format; the lease is tested in one process — two orchestrator processes could both claim a job in the requeue window (the deployment is one process).

### Looking at it

`docs/artifact-studio/evidence/2026-09-12-*.png`: the four-file group at 1440 px (cards aligned with the assistant column, header notes once, Download all), the grid panel at 1440 px and 400 px (sticky header, row numbers, column letters, "34 rows · 9 columns"), the deck's page panel (thumbnails, zoom, page 1/9), the CSV dataset card (retaken on the final tree: the one-sheet CSV keeps the workbook's title, "CSV · 49 KB · 500 rows · 11 columns"). What looking found and fixed: a "With notes" chip repeated on every card, a one-sheet CSV titled "Book — Sheet", dates shown as `T00:00:00`, a two-line per-cell note ahead of the cards.
