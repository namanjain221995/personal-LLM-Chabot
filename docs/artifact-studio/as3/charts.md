...
- **Defaults.** Style-guide tokens are used unless `ResolvedStyle.chart_defaults` exists (duck-typed). The palette order is `#2F6FB2 #E07B00 #0E9D9A #C0566B #6D5AE6 #8A5A44 #3F8F4F #5F6B7A`. The test measures the first five as pairwise at least 25 apart in CIELAB under simulated deuteranopia (Machado 2009, severity 1).
