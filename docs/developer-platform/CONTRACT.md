...
| `GET …/derived`, `…/derived/{name}` | names from a closed list per kind; another name `404 file_not_found`, `param: name`; the download headers above with the output's own type and no `ETag`; while the file is assembling, queued or processing, `409 file_not_ready` with `Retry-After: 5`; a file in `error` (processing failed, an unsupported kind, a failed assembly) has no derived data and is `400 invalid_request_error`, `param: file_id`, whose message is "This file has no derived data: " and the File object's own `status_details` — not retry-safe |
...
| `file_not_ready` | 409 | bytes still assembling; derived data while the file is assembling, queued or processing — `Retry-After`, retry-safe. Never sent for a file in `error`, which is `400` (§8.7) |
...
every request carrying it during its quarantine (§8.4); the edge's
