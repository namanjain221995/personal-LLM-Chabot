...
| F029 | P2 | CONFIRMED | authn-authz | An ADMIN can read a SUPER_ADMIN's (and a peer admin's) conversations, uploads and reports — the … | **FIXED** | `admin_api.py` `_inspectable_member` applies the rank rule on every member content and session read. Test `test_an_admin_cannot_read_a_super_admins_conversations_uploads_or_reports_but_can_read_a_members`. Owner decision 2026-09-14: the rule is now `rbac.may_inspect`, which also lets a super admin read a peer super admin (still audited); admins stay refused (`test_a_super_admin_reads_a_peer_super_admins_content_and_every_read_is_audited`). With that decision the list reads and the member-detail stats of another member are audited as well (`admin_listed_conversations`, `admin_listed_uploads`, `admin_listed_reports`, `admin_listed_sessions`, `admin_viewed_member_stats`; paging and counts only, identical repeats within 60 s coalesced, self-views not recorded; docs/AUTH.md). Tests `test_list_and_stats_reads_of_another_member_are_audited_with_paging_and_counts_only`, `test_repeating_the_same_list_read_is_folded_but_another_page_is_not`, `test_reading_your_own_lists_and_stats_writes_no_event`. | admin surface hardening wave |
...
| N007 | P3 | FOUND IN VERIFICATION | authn-authz | GET /admin/api/members/{user_id}/sessions has no outranks guard - an admin can list a super … | **FIXED** | Same `_inspectable_member` change as F029. Test `test_an_admin_cannot_list_or_revoke_a_super_admins_sessions_but_can_for_a_member`; super admin to peer super admin listing allowed since 2026-09-14, revoke still refused (`test_a_super_admin_may_list_a_peer_super_admins_sessions_but_still_not_revoke_them`). Listing another member's sessions now writes `admin_listed_sessions` (count only, no IPs or user agents in the event). | admin surface hardening wave |
