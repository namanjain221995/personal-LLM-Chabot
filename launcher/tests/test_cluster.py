...
class ValidationTests(unittest.TestCase):
    ...
    def test_gdn_prefill_backend_renders_like_the_speculative_config(self) -> None:
        ...
        Verified inside the pinned build and the candidate image
        (engine/arg_utils.py: ``--gdn-prefill-backend``, choices flashinfer /
        triton / cutedsl). Empty means the flag is absent, never an empty
        argument.
        """
        for values in (dual(), dual(CLUSTER_GDN_PREFILL_BACKEND=""), dual(CLUSTER_GDN_PREFILL_BACKEND="  ")):
            self.assertNotIn("--gdn-prefill-backend", resolve(values)["CLUSTER_ENGINE_ARGS"])
        enabled = resolve(dual(CLUSTER_GDN_PREFILL_BACKEND="flashinfer"))
        argv = shlex.split(enabled["CLUSTER_ENGINE_ARGS"])
        self.assertEqual(argv[argv.index("--gdn-prefill-backend") + 1], "flashinfer")
        self.assertEqual(enabled["CLUSTER_GDN_PREFILL_BACKEND"], "flashinfer")
        # After the manifest's startup arguments, before chunked prefill, so the
        # rendered line reads in the order the engine documents its flags.
        self.assertIn("--attention-backend flashinfer --gdn-prefill-backend flashinfer --enable-chunked-prefill", enabled["CLUSTER_ENGINE_ARGS"])
        self.assertEqual(resolve(dual(CLUSTER_GDN_PREFILL_BACKEND="Triton"))["CLUSTER_GDN_PREFILL_BACKEND"], "triton")
        for bad in ("flash", "auto", "fla", "1"):
            with self.subTest(value=bad), self.assertRaisesRegex(TechSaraError, "CLUSTER_GDN_PREFILL_BACKEND"):
                resolve(dual(CLUSTER_GDN_PREFILL_BACKEND=bad))
        self.assertEqual(gdn_prefill_backend_argument({}), "")
        self.assertEqual(gdn_prefill_backend_argument({"CLUSTER_GDN_PREFILL_BACKEND": "cutedsl"}), "--gdn-prefill-backend cutedsl")

    def test_engine_process_environment_is_rendered_only_when_set(self) -> None:
        """An EMPTY VLLM_ALLREDUCE_USE_FLASHINFER is int("") inside vLLM: never render it."""
        self.assertEqual(engine_process_environment({}), {})
        self.assertEqual(engine_process_environment({"CLUSTER_VLLM_ALLREDUCE_USE_FLASHINFER": ""}), {})
        both = engine_process_environment(
            {"CLUSTER_VLLM_ALLREDUCE_USE_FLASHINFER": "0", "CLUSTER_VLLM_USE_V2_MODEL_RUNNER": "1"}
        )
        self.assertEqual(both, {"VLLM_ALLREDUCE_USE_FLASHINFER": "0", "VLLM_USE_V2_MODEL_RUNNER": "1"})
        for key in ("CLUSTER_VLLM_ALLREDUCE_USE_FLASHINFER", "CLUSTER_VLLM_USE_V2_MODEL_RUNNER"):
            for bad in ("true", "2", "yes"):
                with self.subTest(key=key, value=bad), self.assertRaisesRegex(TechSaraError, key):
                    engine_process_environment({key: bad})
        rendered = render_engine_env(both)
        self.assertTrue(rendered.startswith("# Generated by the launcher"))
        self.assertIn("\nVLLM_ALLREDUCE_USE_FLASHINFER=0\nVLLM_USE_V2_MODEL_RUNNER=1\n", rendered)
        self.assertNotIn("=\n", render_engine_env({}), "an empty file carries no empty assignment")
        # Recorded on the generated line for the record, sorted, only when set.
        self.assertEqual(
            resolve(dual(CLUSTER_VLLM_USE_V2_MODEL_RUNNER="0", CLUSTER_VLLM_ALLREDUCE_USE_FLASHINFER="0"))["CLUSTER_ENGINE_ENV"],
            "VLLM_ALLREDUCE_USE_FLASHINFER=0 VLLM_USE_V2_MODEL_RUNNER=0",
        )

    def test_sentinel_autonomy_healthcheck_tier_and_worker_management_address(self) -> None:
        self.assertEqual(resolve(dual(CLUSTER_SENTINEL_AUTONOMOUS="1"))["CLUSTER_SENTINEL_AUTONOMOUS"], "1")
        with self.assertRaisesRegex(TechSaraError, "CLUSTER_SENTINEL_AUTONOMOUS"):
            resolve(dual(CLUSTER_SENTINEL_AUTONOMOUS="true"))
        self.assertEqual(resolve(dual(VLLM_HEALTHCHECK_KILL_AFTER="12"))["VLLM_HEALTHCHECK_KILL_AFTER"], "12")
        for bad in ("1", "0", "121", "many"):
            with self.subTest(value=bad), self.assertRaisesRegex(TechSaraError, "VLLM_HEALTHCHECK_KILL_AFTER"):
                resolve(dual(VLLM_HEALTHCHECK_KILL_AFTER=bad))
        self.assertEqual(resolve(dual(CLUSTER_WORKER_MGMT_IP="192.168.9.68"))["CLUSTER_WORKER_MGMT_IP"], "192.168.9.68")
        # monitoring.sh's older spelling of the same address is honoured, and a
        # hostname there is not a cluster error (that script resolves it).
        self.assertEqual(resolve(dual(MONITORING_WORKER_BIND="192.168.9.68"))["CLUSTER_WORKER_MGMT_IP"], "192.168.9.68")
        self.assertEqual(resolve(dual(MONITORING_WORKER_BIND="spark-2"))["CLUSTER_WORKER_MGMT_IP"], "")
        self.assertEqual(
            resolve(dual(CLUSTER_WORKER_MGMT_IP="192.168.9.68", MONITORING_WORKER_BIND="10.0.0.9"))["CLUSTER_WORKER_MGMT_IP"],
            "192.168.9.68",
        )
