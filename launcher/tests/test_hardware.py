...
class CachePathTests(unittest.TestCase):
    ...
    default.  Removing exactly the keys the function reads, and restoring them
    afterwards, makes both boxes give the same answer.
    """

    #: Every environment key ``choose_cache_path`` consults, in the order it
    #: consults them.  Keep in step with ``techsara_cli.hardware``.
    CACHE_ENVIRONMENT_KEYS = (
        "TECHSARA_MODEL_CACHE",
        "VLLM_MODELS_DIR",
        "LOCALAPPDATA",
        "XDG_CACHE_HOME",
    )
    ...
    def setUp(self) -> None:
        ...
        for key in self.CACHE_ENVIRONMENT_KEYS:
