...
def _md_escape(value: Any, limit: int = 60) -> str:
    ...
    text = text.replace("[", "\\[").replace("]", "\\]")
    # MARKUP CHARACTERS ARE ESCAPED. A cell "2*3*4" rendered as 234, "x^2^" and
    # "H~2~O" as super/subscript, backticks as code and quotes turned curly
    # (review 2026-09-19): a report must show the file's own text.
    for ch in "*`~^$\"'":
...
def _caveat(value: Any) -> str:
    ...
    # _md_escape already escapes "*"; only the underscore is added here.
    return _md_escape(value, limit=400).replace("_", "\\_")
