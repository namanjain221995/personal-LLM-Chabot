...
def _stream_description() -> str:
    ...
    return (
        ...
        "is waiting. A request that names files also gets one "
        "`response.output_text.annotation.added` event per `file_citation` "
        "after `response.output_text.done`, and `response.completed` carries "
        "the same annotations on its `output_text` part. "
        "The terminals are `response.completed`, `response.failed` "
