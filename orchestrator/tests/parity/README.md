...
failing build leaves a floor-based test green; it cannot leave these green,
and the failure names the check that moved and what it moved from. Those five
numbers — 10, 10, 11, 12, 14 — are the proof this harness entered the
repository without being softened.
...
| `test_parity.py` | the calibration guards and the exact-score pins |
